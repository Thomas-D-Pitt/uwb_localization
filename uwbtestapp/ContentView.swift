...
struct ContentView: View {
    ...
    @State private var settings_certainty_loss = ""
    @State private var settings_sd = ""
    ...
    var body: some View {
        ...
        ZStack{
            ...
            .alert(String("Settings"), isPresented: $settingsAlert){
                ...
                TextField(String(format:"Certainty Loss (c: %.3f)", UWBManager.certaintyLoss), text: $settings_certainty_loss)
                TextField(String(format:"Standard Deviation (c: %.3f)", UWBManager.sd), text: $settings_sd)
                Button("reset BT", action: {() in self.resetBT()})
    ...
    func updateSettings(){
        ...
        UWBManager.certaintyLoss = Double(settings_certainty_loss) ?? UWBManager.certaintyLoss
        settings_certainty_loss = ""
        UWBManager.sd = Double(settings_sd) ?? UWBManager.sd
        settings_sd = ""
    }
    
    func resetBT(){
        print("starting BT")
        uwb.BTConnection.startCentralManager()
        _ = Timer.scheduledTimer(timeInterval: 0.25, target: self.uwb, selector: #selector(self.uwb.BTLocationUpdate), userInfo: nil, repeats: true)
...
class UWBManager : ObservableObject{
    ...
    public var BTConnection = BLEConnection()
    public var BTUpdating = false

    let radiusBuffer = 0.05
    let smoothing_const = 0.0
    
    var xFilter = KalmanFilter(stateEstimatePrior: 0.0, errorCovariancePrior: 1.0)
    var yFilter = KalmanFilter(stateEstimatePrior: 0.0, errorCovariancePrior: 1.0)
    static var sd = 0.1
    static var certaintyLoss = 0.1
    ...
    @objc func BTLocationUpdate(){
        BTConnection.writeOutgoingValue(data: String(atX) + "," + String(atY))
    }
...
//                print("---")
        ...
        if at.dist >= 0.0{
            ...
            let xPrediction = xFilter.predict(stateTransitionModel: 1, controlInputModel: 0, controlVector: 0, covarianceOfProcessNoise: UWBManager.certaintyLoss)
            let xUpdate = xPrediction.update(measurement: newX, observationModel: 1, covarienceOfObservationNoise: UWBManager.sd)
            xFilter = xUpdate
            atX = xFilter.stateEstimatePrior
            
            let yPrediction = yFilter.predict(stateTransitionModel: 1, controlInputModel: 0, controlVector: 0, covarianceOfProcessNoise: UWBManager.certaintyLoss)
            let yUpdate = yPrediction.update(measurement: newY, observationModel: 1, covarienceOfObservationNoise: UWBManager.sd)
            yFilter = yUpdate
            atY = yFilter.stateEstimatePrior
